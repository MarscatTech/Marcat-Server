...
use storage::{ADMINS, COMMENTS, LIKES, POSTS, POST_COMMENTS, USERS};
use types::{
    Comment, Error, LikeKey, Post, PostCommentKey, PostImage, PostStatus, PostSummary, PostsPage,
    UserProfile, MAX_COMMENT_LENGTH, MAX_CONTENT_LENGTH, MAX_IMAGES_PER_POST,
};
...
fn delete_post(post_id: u64) -> Result<(), Error> {
    ...
    let comment_ids: Vec<u64> = POST_COMMENTS.with(|pc| {
        let pc = pc.borrow();
        let start = PostCommentKey { post_id, comment_id: 0 };
        let end = PostCommentKey { post_id, comment_id: u64::MAX };
        pc.range(start..=end)
            .map(|(k, _)| k.comment_id)
            .collect()
    });
    ...
    POST_COMMENTS.with(|pc| {
        ...
        for cid in &comment_ids {
            pc.remove(&PostCommentKey { post_id, comment_id: *cid });
        }
    });
...
fn like_post(post_id: u64) -> Result<(), Error> {
    ...
    let key = LikeKey { post_id, user: caller };
...
#[update]
fn add_comment(post_id: u64, content: String) -> Result<u64, Error> {
    auth::require_registered()?;
    ...
    POST_COMMENTS.with(|pc| {
        pc.borrow_mut()
            .insert(PostCommentKey { post_id, comment_id: id }, ());
    });
...
fn delete_comment(comment_id: u64) -> Result<(), Error> {
    ...
    let comment = COMMENTS.with(|c| {
        c.borrow().get(&comment_id).ok_or(Error::CommentNotFound)
    })?;
...
fn http_request(req: HttpRequest) -> HttpResponse {
    ...
    if let Some(rest) = path.strip_prefix("/post/") {
        ...
        if parts.len() == 3 && parts[1] == "image" {
            if let (Ok(post_id), Ok(image_index)) = (parts[0].parse::<u64>(), parts[2].parse::<u32>()) {
